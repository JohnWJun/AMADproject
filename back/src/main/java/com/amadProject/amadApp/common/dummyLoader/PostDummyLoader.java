...
public class PostDummyLoader implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        List<String> emails = List.of(
                "happyJoseph@gmail.com",
                "GoodMaria@gmail.com",
                "HolyJohn@gmail.com",
                "BraveDavid@gmail.com"
        );

        List<String> bibles = List.of("john", "psa", "rom", "gen", "mat", "luk", "act");
        ...
        for (int i = 1; i <= 30; i++) {
            ...
            post.setTitle("더미 게시글 " + i);
            post.setContent_1("더미 내용 " + i);
            post.setContent_2("오늘의 묵상 " + i);
            post.setContent_3("하나님과의 동행에 대한 기록 " + i);
            post.setContent_4("믿음과 순종에 대한 고백 " + i);
            post.setContent_5("오늘도 감사함으로 살기 " + i);
            post.setPublishedDate(LocalDateTime.now().minusDays(i));

            // 3️⃣ Amad 생성
            Amad amad = new Amad();
            amad.setPost(post);
            amad.setMember(member);
            amad.setMission("오늘의 미션 " + i + ": 감사 3가지 적기");
            ...
            bible.setPost(post);
            bible.setBible(bibles.get(i % bibles.size()));
            bible.setBibleChapter((i % 20) + 1);
            bible.setBibleVerseFrom((i % 30) + 1);
            bible.setBibleVerseTo((i % 30) + 1);

            // 5️⃣ 저장
            repository.save(post);
            amadRepository.save(amad);
            bibleChapterVerseRepository.save(bible);
...
//        josephPost.setPublishedDate(date1);
//
